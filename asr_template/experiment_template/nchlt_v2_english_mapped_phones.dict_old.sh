...
DIR_G2P=~/hlt/english/Dictionaries.Lwazi.Eng.1.2/g2p
...
NEW_WORDS=$1
NEW_DICT=$DIR_ROOT/dicts/eng.corpus.dict
...
if [ $DO_CHECK_RULES == 1 ] || [ $DO_ALL = 1 ]; then
  ...
  die_if_error
  $DIR_DICT_SCRIPTS/check_rules.pl gra $DIR_G2P/english.rules $NEW_WORDS.graphs $NEW_WORDS.g-check
  die_if_error
...
if [ $DO_DICT == 1 ] || [ $DO_ALL == 1 ]; then
  ...
  $DIR_DICT_SCRIPTS/create_dict.pl $NEW_WORDS $DIR_G2P/english.rules $DIR_G2P/english.gnulls $NEW_DICT.tmp1 -p $DIR_G2P/english.map.phones rtl
  die_if_error
  echo "h	h" >> $NEW_DICT.tmp1
  
    echo "Map to Sepedi phonemes and sort"
  $DIR_DICT_SCRIPTS/remap_dict.pl pho $NEW_DICT.tmp1 /media/DATA/nchlt_sepedi_20111004_20111222_V2/english2sepedi.countmatrix.nchlt.map ltr $NEW_DICT.tmp2
  die_if_error
 
  echo "Map to HTK-compatible phonemes and sort"
  $DIR_DICT_SCRIPTS/remap_dict.pl pho $NEW_DICT.tmp2 /home/thipe/LWAZI/asr/dicts/g2p/sampa2htk.map ltr $NEW_DICT.tmp3
  die_if_error
  ...
  cp -v $NEW_DICT $DIR_ROOT/dicts/nchlt_v2_eng_1.2_mapped2nso.dict
