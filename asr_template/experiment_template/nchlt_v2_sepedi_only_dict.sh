...
DIR_TRANSCRIPTS=/media/DATA/nchlt_sepedi_20111004_20111222_V2/transcriptions
DIR_DICT_SCRIPTS=~/LWAZI/asr.sprint.Dec.2010/scripts

DIR_ROOT=/media/DATA/nchlt_sepedi_20111004_20111222_V2
DIR_ASR_SCRIPTS=~/hlt/sepedi/asr_template

NEW_WORDS=$DIR_ROOT/dicts/sepedi.corpus.words.only
NEW_DICT=$DIR_ROOT/dicts/nchlt_v2_sepedi_only.dict
...
for dirName in $DIR_ROOT/processed_trans $DIR_ROOT/dicts $DIR_ROOT/lists $DIR_ROOT/tmp; do
  if [ ! -d $dirName ]; then
    mkdir -p $dirName
  fi
  die_if_error
...
#  gawk '{print $2}' < $DIR_ROOT/lists/preprocess.lst > $DIR_ROOT/lists/txt.lst
#  $DIR_ASR_SCRIPTS/src/create_wordlist.pl $DIR_ROOT/lists/txt.lst $NEW_WORDS
#  die_if_error
#fi
#------------------------------------------------------------------------------
...
if [ $DO_DICT == 1 ] || [ $DO_ALL == 1 ]; then
  ...
  $DIR_DICT_SCRIPTS/create_dict.pl $NEW_WORDS $DIR_ROOT/tmp/sepedi.rules.sorted $DIR_G2P/sepedi.gnulls $NEW_DICT.tmp1 -p $DIR_G2P/sepedi.map.phones rtl
  die_if_error
  #echo "h	h" >> $NEW_DICT.tmp1

  echo "Africates splitting"
  $DIR_DICT_SCRIPTS/remap_dict.pl pho $NEW_DICT.tmp1 /home/thipe/LWAZI/asr/dicts/corpus_dictionaries/affricates.map ltr $NEW_DICT.tmp2
  die_if_error

  echo "Map to HTK-compatible phonemes and sort"
  $DIR_DICT_SCRIPTS/remap_dict.pl pho $NEW_DICT.tmp2 /home/thipe/LWAZI/asr/dicts/g2p/sampa2htk.map ltr $NEW_DICT.tmp2
  die_if_error
  sed -i s/1/š/g $NEW_WORDS
